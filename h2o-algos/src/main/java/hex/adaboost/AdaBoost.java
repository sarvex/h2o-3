...
public class AdaBoost extends ModelBuilder<AdaBoostModel, AdaBoostModel.AdaBoostParameters, AdaBoostModel.AdaBoostOutput> {
    ...
    private class AdaBoostDriver extends Driver {
        ...
        private void buildAdaboost() {
            ...
            Scope.track(train());
            System.out.println(train().toTwoDimTable(0,10,false));
            System.out.println("train()._key = " + train()._key);
            for (int n = 0; n < _parms._n_estimators; n++) {
                ...
                SharedTreeSubgraph sts = drf.getSharedTreeSubgraph(0,0);
                System.out.println("sts = " + sts);
                _model._output.models[n] = drf._key;
                Frame score = drf.score(train());
                Scope.track(score);
                System.out.println(score.toTwoDimTable(0, 10, false));
                System.out.println(train().vec(_parms._response_column).toTwoDimTable(0, 10));
                ...
                System.out.println("e_m = " + e_m);
                double alpha_m = _parms._learning_rate * Math.log((1 - e_m) / e_m);
                _model._output.alphas[n] = alpha_m;
                System.out.println("alpha_m = " + alpha_m);

                UpdateW updateW = new UpdateW(alpha_m).doAll(train().vec("weights"), train().vec(_parms._response_column), score.vec("predict"));
                System.out.println(train().toTwoDimTable(0, 10, false));
            }
        }
    }
    ...
    private class UpdateW extends MRTask<UpdateW> {
        ...
        public UpdateW(double alpha_m) {
            ...
            System.out.println("exp_am = " + exp_am);
            System.out.println("exp_am_inverse = " + exp_am_inverse);
        }
